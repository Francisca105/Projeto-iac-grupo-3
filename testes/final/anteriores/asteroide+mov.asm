...
POSSIVEIS_AST:
    WORD    0, 1                            ; coluna inicial e movimento horizontal
    WORD    30, -1
    WORD    30, 0
    WORD    30, 1
    WORD    59, -1
...
asteroides_int:
	...
	MOV  [R0], R1		                    ; na componente 2 da variável evento_int
    POP     R1
	POP  R0
	RFE
...
coloca_topo:
    ...
    JZ      bomm                            ; se sim, procede como tal
...
teste2:
    ...
    MOV     R4, R0
    SHL     R4, 2                           ; valor a adicionar (nº do asteróide)
    MOV     R0, POS_AST                     ; posição dos asteróides
    ADD     R0, R4                          ; posição do asteróide
    MOV     R1, ASTEROIDES                  ; tabela dos asteróides
    ADD     R1, R4                          ; asteróide a tratar
    MOV     R3, R1                          ; copia o valor
    ADD     R1, 2                           ; tipo do asteróide
    MOV     R2, AST_BOM                     ; simboliza um asteróide bom
    MOV     R1, [R1]                        ; tipo do asteróide
    CMP     R1, R2                          ; o asteróide é bom?
    JZ      bommm                           ; se sim, procede como tal
    MOV     R1, DEF_ASTEROIDE_MAU           ; definição do asteróide mau
exit_bommm:
    MOV     R2, MOV_AST                     ; movimentos dos asteróides
    ADD     R2, R4                          ; movimentos do asteróide    
    MOV     R3, [R3]                        ; ecrã do asteróide
    CALL    move_objeto
