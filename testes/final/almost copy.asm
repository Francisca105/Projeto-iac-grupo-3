...
exit_coloca_bom:  
    ...
    JMP     verifica_asteroide              ; verifica se o asteroide já existe na mesma lane e numa linha proxima
...
verifica_asteroide_loop:
    MOV     R8, ASTEROIDES                  ; endereço da tabela dos asteroides
    MOV     R9, POS_AST                     ; endereço da tabela das posições dos asteroides
    ...
    MOV     R10, R7                         ; copia o valor do contador
    SHL     R10, 2                          ; multiplica por 4 (nº de variáveis por posição do asteroide)
    ADD     R9, R10                         ; endereço da posição do asteroide a tratar

    MOV     R10, [R8+4]                     ; estado do asteroide
    CMP     R10, ON                         ; o asteroide está ativo (?)
    JZ      verifica_asteroide_next         ; se não, verifica o próximo asteroide

    MOV     R10, [R9+6]                     ; lane do asteroide
    CMP     R10, R3                         ; o asteroide está na mesma lane (?)
    JNZ     verifica_asteroide_next         ; se não, verifica o próximo asteroide

    MOV     R10, [R9]                       ; linha do asteroide
    MOV     R9, LINHA_MINIMO                ; linha mínima
    CMP     R9, R10                         ; o asteroide está abaixo da linha mínima (?)
    JLE     verifica_asteroide_next         ; se não, verifica o próximo asteroide

    JMP     exit_verifica_asteroide_repor   ; sai do ciclo


verifica_asteroide_next:
    ADD     R7, 1                           ; incrementa o contador
    JMP     verifica_asteroide_loop         ; verifica o próximo asteroide

exit_verifica_asteroide_repor:
    POP     R10
    POP     R9
    POP     R8
    POP     R7
...
exit_verifica_asteroide_bom:
    POP     R10
    POP     R9
    POP     R8
    POP     R7
    JMP     exit_coloca_bom_2
